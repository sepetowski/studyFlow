...
export const ToolsContainer = ({ editor }: Props) => {
	return (
		<div className=' rounded-md  shadow-sm border bg-popover p-1 text-popover-foreground flex items-center gap-1 '>
			<OptionBtn
			...
			<OptionBtn onClick={() => editor.chain().focus().unsetColor().run()} data-testid='unsetColor'>
				<span className='w-4 h-4 rounded-full bg-secondary-foreground'></span>
			</OptionBtn>
			<Separator className='h-6' orientation='vertical' />
			<OptionBtn onClick={() => editor.commands.undo()}>
				<Undo2 size={16} />
			</OptionBtn>
			<OptionBtn onClick={() => editor.commands.redo()}>
				<Redo2 size={16} />
			</OptionBtn>
		</div>
	);
